...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
  ...
  long row, col, kk, i, j, kkk;
  //double sum;                                                                                                                                                                                                   
  //#pragma omp parallel for private(col, kk, i, j, kkk)                                                                                                                                                          
    ...
    for (col = 0; col < k; col += BLOCK_SIZE){
      for (kk = 0; kk < m; kk += BLOCK_SIZE){
        for (i = row; i < row + BLOCK_SIZE; i++){
          ...
          for (kkk = kk; kkk < kk+BLOCK_SIZE; kkk++){
            c[i+j*m] += a[i+kkk*m]*b[kkk + j*k];
        }                                                                                                                                                                                     
        }
...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    double bandwidth = 2*m*n*(k*BLOCK_SIZE+1)*NREPEATS/(BLOCK_SIZE*BLOCK_SIZE)/sizeof(double)/1e9/time; // TODO: calculate from m, n, k, NREPEATS, time
